...
func (t *AlterableDoltTable) CreateForeignKey(ctx *sql.Context, fkName string, columns []string, referencedTable string, referencedColumns []string,
	...
	realColNames := make([]string, len(columns))
	...
	realRefColNames := make([]string, len(referencedColumns))
	...
	for i, col := range referencedColumns {
		...
		realRefColNames[i] = tableCol.Name
		...
		if !tblCols[i].TypeInfo.Equals(tableCol.TypeInfo) {
			...
			return fmt.Errorf("column type mismatch on `%s` and `%s`", realColNames[i], tableCol.Name)
		}
	}
	...
	newTable, _, tableIndex, err := t.createIndex(ctx, t.table, t.sch, true, "", sql.IndexUsing_Default, sql.IndexConstraint_None, sqlColNames, "")
	if err != nil {
		return err
	}
	newRefTable, _, refTableIndex, err := t.createIndex(ctx, refTable, refTableSch, true, "", sql.IndexUsing_Default, sql.IndexConstraint_None, sqlRefColNames, "")
	if err != nil {
		return err
	}
	...
	_, err = foreignKeyCollection.AddForeignKey(fkName, t.name, tableIndex.Name(), realColNames, refTableName, refTableIndex.Name(), realRefColNames, onDeleteRefOp, onUpdateRefOp)
	if err != nil {
		return err
	}
...
func (t *AlterableDoltTable) DropForeignKey(ctx *sql.Context, fkName string) error {
	...
	foreignKey, err := fkc.RemoveForeignKey(fkName)
	if err != nil {
		return err
	}
	...
	otherTable, ok, err := newRoot.GetTable(ctx, foreignKey.ReferencedTableName)
	if err != nil {
		return err
	}
	if !ok {
	...
	otherSch, err := otherTable.GetSchema(ctx)
	if err != nil {
		return err
	}
	newOtherTable, _, err := t.dropIndex(ctx, otherTable, otherSch, foreignKey.ReferencedTableIndex)
	if err != nil {
		return err
	}
	newRoot, err = root.PutTable(ctx, foreignKey.ReferencedTableName, newOtherTable)
	if err != nil {
		return err
	}
...
func (t *AlterableDoltTable) createIndex(ctx *sql.Context, tbl *doltdb.Table, tblSch schema.Schema, hidden bool, indexName string,
	using sql.IndexUsing, constraint sql.IndexConstraint, columns []sql.IndexColumn, comment string) (*doltdb.Table, schema.Schema, schema.Index, error) {
	...
	var err error
	if !hidden {
		index, err = tblSch.Indexes().AddIndexByColNames(indexName, realColNames, constraint == sql.IndexConstraint_Unique, comment, false)
	} else {
		indexName := "dolt_fk_1"
		for i := 2; tblSch.Indexes().Contains(indexName); i++ {
			indexName = fmt.Sprintf("dolt_fk_%d", i)
		}
		index, err = tblSch.Indexes().AddIndexByColNames(indexName, realColNames, constraint == sql.IndexConstraint_Unique, comment, true)
	}
	if err != nil {
